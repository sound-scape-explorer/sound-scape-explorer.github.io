...
> `{app}_vis` is only available if you have installed the `Processing` module.
...
```bash
# Windows
{app}_vis C:\path\to\config.xlsx

# Ubuntu / macOS
(venv)$> {app}_vis /path/to/config.xlsx
```
...
> **Note**
>
> You can also use `{app}_viz` or `viz` as aliases to the `{app}_vis` command.
